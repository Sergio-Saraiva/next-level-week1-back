...
class PointsController{
  async create(req: Request, res: Response){
    ...
    // const dbItems = await knex('items').select('id')

    // let dbItemsArray = Array()

    // dbItems.forEach(i => {
    //   dbItemsArray.push(i.id)
    // })

    // let aux = 0;

    // items.forEach((i: number) => {
    //   if(!(dbItemsArray.includes(i))){
    //     aux++
    //   }
    // })
  ...
  async index(req: Request, res: Response) {
    ...
    const serializedPoints = points.map(point => {
      return {
        ...point,
        image_url: `http://192.168.1.164:3333/uploads/${point.image}`
      }
    })
